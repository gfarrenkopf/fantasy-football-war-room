...
**The Opening Night Exception.** The two bookends of the night — this overlay and the Final Whistle — are the only places the product deliberately breaks its own muted grammar: a second typeface, filled saturated slabs and buttons, a text halo, stage lighting, screen shake and confetti. The exception is scoped to those two overlays and those two moments — the start of a draft someone has waited a season for, and the team they built by the end of it — and it buys nothing anywhere else. Every number on either is the league's and the user's own; each is shown once per league, and each is always skippable.

### Final Whistle (`/draft`, once per league, when the last pick lands)
