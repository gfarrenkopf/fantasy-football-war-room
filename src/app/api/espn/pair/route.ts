...
interface PairRequest {
  ...
  espnTeamId: number;
  season: number;
  /** The disclosure version the user just acknowledged in the popup, if they had to. */
  acknowledged?: number;
}
...
function parsePairRequest(body: unknown): PairRequest | null {
  ...
  const acknowledged = Number.isInteger(b.acknowledged) ? b.acknowledged : undefined;
  return { leagueId: b.leagueId, espnLeagueId: b.espnLeagueId, espnTeamId: b.espnTeamId!, season: b.season!, acknowledged };
}

/**
 * POST /api/espn/pair { leagueId, espnLeagueId, espnTeamId, season, acknowledged? } → { token, expiresAt }
 *
...
export const POST = withUser(async (request, _ctx, { db, userId, email }) => {
  ...
  if (!(await hasAcknowledgedDisclosure(db, userId, ESPN_DISCLOSURE_VERSION))) {
    if (pair.acknowledged !== ESPN_DISCLOSURE_VERSION) return json(428, { needsDisclosure: ESPN_DISCLOSURE_VERSION });
