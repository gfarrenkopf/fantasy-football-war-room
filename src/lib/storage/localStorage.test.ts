...
describe("prefs store", () => {
  it("round-trips", async () => {
    const stores = createLocalStores(memoryStorage());
    const prefs: UiPrefs = { view: "board", center: "plan", baMode: "all", mockOn: true, room: ["casual", "sharp"], activeLeagueId: "a", premiered: ["a"], wrapped: [] };
...
describe("league store", () => {
  ...
  it("keeps a namespace's leagues and drafts apart but shares prefs", async () => {
    ...
    await anon.prefs.savePrefs({ view: "board", center: "ba", baMode: "pos", mockOn: false, room: null, activeLeagueId: null, premiered: null, wrapped: [] });
