 ...
 */
export function withUser<Ctx>(handler: (request: Request, ctx: Ctx, auth: { db: Db; userId: string }) => Promise<Response>) {
  return async (request: Request, ctx: Ctx): Promise<Response> => {
    ...
    const response = await handler(request, ctx, { db: getDb(), userId: user.userId });
