...
**Deployment.** The hosted app runs on one droplet without Docker: `deploy/warroom.service` (systemd, `next start` on localhost), `deploy/Caddyfile` (TLS), and `deploy/deploy.sh`, which builds each commit into `releases/<sha>`, backs up and migrates the database, then swaps the `current` symlink (`deploy.sh rollback` swaps back; migrations aren't reverted). `deploy/warroom-backup.timer` backs up nightly. `.github/workflows/deploy.yml` deploys `main` after CI passes, over SSH through the forced command `deploy/ci-deploy.sh`. The runbook is `docs/deployment.md`; `docker-compose.dev.yml` is only for local Postgres.
